...
def Quant_prob(x, cdf_table=None, H=60, levels=128, mode='software'):
...
class QuantizeLinear(nn.Linear):
    ...
    def __init__(self, *kargs, **kwargs):
        ...
        #print("This is the actual definition of the linear layer")
        #print("SRAM DEPTH LINEAR: ",self.sram_depth)
                ...
                for input_p, weight_p in zip(input_list, self.weight_list):
                    ...
                    for ii in range(self.a_bits):
                        for iw in range(self.w_bits):
                          ...
                          partial_sum_quantized = Quant_prob(partial_sum, cdf_table=self.cdf_table, H=self.quant_bound, mode=self.mode, levels=self.adc_levels)
...
class QuantizeConv2d(nn.Conv2d):
    def __init__(self, *kargs, **kwargs):
        ...
        #print("SRAM DEPTH FOR CONV : ",self.sram_depth)
        #print("This is the actual layer definition")

    def dec_to_binary(my_int):
    
      if my_int < 4096:
          #return "{0:012b}".format(my_int)
          return np.binary_repr((my_int), width=3)
      else:
          return "111111111111"
    ...
    def forward(self, input):
        ...
        #print("Weights Original Shape:",self.weight.org.shape)
        #np.save('self_weight_org.npy', self.weight.org.cpu().numpy())
        #TODO: Add dump functionality for original weights in the similar manner as implemented in resnet_quant.py
        ...
        # Quantize weights to w_bits
        self.weight.data=Quantize(self.weight.org, n_bits=self.w_bits)
        #print("Weights Quantized Shape:",self.weight.data.shape)
        #np.save('self_weight_quan.npy', self.weight.data.cpu().numpy())
        #TODO: Add dump functionality for quantized weights in the similar manner as implemented in resnet_quant.py
